...
{
  LogMessage *clone = log_msg_ref(msg);
  clone = log_msg_make_writable(&clone, path_options);
...
/*
 * This tests that the clone does not break the acknowledgement
 * or the reference counting independently, whether the cloned 
 * message has own payload or not
 */
ParameterizedTest(struct nv_pair *param, msg_ack, test_cloned_clone)
{
  ...
  log_msg_set_value_by_name(clone, param->name, param->value, -1);
  log_msg_write_protect(clone);

  LogMessage *cloned_clone1 = create_clone(clone,  &t->path_options);
  LogMessage *cloned_clone2 = create_clone(clone,  &t->path_options);
