...
static LogPipe *
_clone(LogProcessPipe *s)
{
  KVParser *self = (KVParser *) s;
  LogParser *cloned = kv_parser_new(s->super.cfg);

  return _set_cloned_fields(cloned, self);
}

static LogPipe *
_clone_linux_audit(LogProcessPipe *s)
{
  KVParser *self = (KVParser *) s;
  LogParser *cloned = kv_parser_linux_audit_new(s->super.cfg);
...
kv_parser_linux_audit_new(GlobalConfig *cfg)
{
  ...
  self->super.super.super.init = _init_linux_audit;
